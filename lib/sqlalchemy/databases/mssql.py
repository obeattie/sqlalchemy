...
class MSSQLDialect(default.DefaultDialect):
    ...
    def get_table_names(self, connection, schema, info_cache=None):
        import sqlalchemy.databases.information_schema as ischema
    ...
    def get_view_names(self, connection, schema, info_cache=None):
        import sqlalchemy.databases.information_schema as ischema
        if schema is not None:
            current_schema = schema
        else:
    ...
    def get_indexes(self, connection, table_name, schema=None,
        ...
        if schema is not None:
            ...
            current_schema = self.get_default_schema_name(connection)
        full_tname = "%s.%s" % (schema, table_name)
        indexes = []
    ...
    def get_columns(self, connection, table_name, schema=None, info_cache=None):
        ...
        while True:
            ...
            if default is not None:
                colargs.append(sqlalchemy.schema.DefaultClause(sql.text(default)))
